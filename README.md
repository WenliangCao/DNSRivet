...
| --- | ---: |
| Release binary | 2,472,544 bytes (2.36 MiB) |
...
```bash
sudo ./target/release/dnsrivet start --config example.config.toml
./target/release/dnsrivet status
sudo ./target/release/dnsrivet restart
sudo ./target/release/dnsrivet stop
sudo ./target/release/dnsrivet uninstall
```
...
- Debug logs can contain queried domain names.
...
| 测量项 | 实测值 |
| --- | ---: |
| release 二进制 | 2,472,544 bytes（2.36 MiB） |
